package com.cadrikmdev.core.connectivty.presentation.network
        ...
        return callbackFlow {
            val callback = object : ConnectivityManager.NetworkCallback() {
                override fun onAvailable(network: Network) {
                    super.onAvailable(network)
                    launch {
                        send(com.cadrikmdev.core.connectivty.domain.connectivity.ConnectivityObserver.Status.AVAILABLE)
                    }
                }
                ...
                override fun onLosing(network: Network, maxMsToLive: Int) {
                    super.onLosing(network, maxMsToLive)
                    launch {
                        send(com.cadrikmdev.core.connectivty.domain.connectivity.ConnectivityObserver.Status.LOSING)
                    }
                }
                ...
                override fun onLost(network: Network) {
                    super.onLost(network)
                    launch {
                        send(com.cadrikmdev.core.connectivty.domain.connectivity.ConnectivityObserver.Status.LOST)
                    }
                }
                ...
                override fun onUnavailable() {
                    super.onUnavailable()
                    launch {
                        send(com.cadrikmdev.core.connectivty.domain.connectivity.ConnectivityObserver.Status.UNAVAILABLE)
                    }
                }
            }
            connectivityManager.registerDefaultNetworkCallback(callback)
